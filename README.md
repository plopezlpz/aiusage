...
| --- | --- | --- | --- |
| macOS | Yes | Claude, Codex, Kimi | arm64, macOS 13+ |
| Linux/Unix | Yes | Claude, Codex, Kimi | No |
...
Provider caches live in the platform user-cache directory (on macOS, `~/Library/Caches/aiusage`). They contain quota values, timestamps, provider metadata, plan/tier details when returned, and sanitized failures. OAuth and local-server tokens are not cached.
...
- `aiusage collect-claude|collect-codex|collect-kimi` — one-shot collection
